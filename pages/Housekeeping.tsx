  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white transition-colors">Staff Tasks</h2>
          <p className="text-slate-500 dark:text-slate-400">Current cleaning and maintenance requirements.</p>
        </div>
        <button className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {tasks.map(room => (
          <div key={room.id} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden transition-colors">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold mt-1">{room.type}</p>
              </div>
              <div className={`p-3 rounded-xl ${room.status === RoomStatus.MAINTENANCE ? 'bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400' : 'bg-[#5B8FB1]/10 dark:bg-[#5B8FB1]/20 text-[#5B8FB1]'}`}>
