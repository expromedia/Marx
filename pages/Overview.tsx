...
const StatCard: React.FC<{ title: string; value: string; icon: any; trend: string; isPositive: boolean }> = ({ title, value, icon: Icon, trend, isPositive }) => (
  <div className="bg-slate-50 dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:scale-[1.02]">
    <div className="flex items-center justify-between mb-4">
      <div className="p-2 bg-[#5B8FB1]/10 dark:bg-[#5B8FB1]/20 text-[#5B8FB1] rounded-lg">
        <Icon size={22} />
      </div>
    ...
    <p className="text-xl sm:text-2xl font-bold mt-1 text-slate-950 dark:text-white">{value}</p>
  </div>
);
...
const Overview: React.FC<{ role: UserRole }> = ({ role }) => {
  return (
    <div className="space-y-6 lg:space-y-8">
      <div>
        <h2 className="text-2xl sm:text-3xl font-bold text-slate-950 dark:text-white tracking-tight leading-tight transition-colors">Welcome back, {role === UserRole.ADMIN ? 'Administrator' : 'Staff Member'}</h2>
        <p className="text-slate-500 dark:text-slate-400 font-medium text-sm sm:text-base">Here's what's happening at Marx Hotel & Suites today.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
          <h3 className="text-base sm:text-lg font-bold mb-6 text-slate-950 dark:text-white">Revenue Performance</h3>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
          <h3 className="text-base sm:text-lg font-bold mb-6 text-slate-950 dark:text-white">Booking Trends</h3>
          <div className="h-64 sm:h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#334155" />
